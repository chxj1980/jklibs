 ...
 */
typedef int (list_apply_h)(struct le *le, void *arg);
 ...
 */
typedef int (list_sort_h)(struct le *le1, struct le *le2, void *arg);
...
struct le *list_apply(const struct list *list, int fwd, list_apply_h *ah,
		      void *arg);
...
static inline int list_contains(const struct list *list, const struct le *le)
{
	return le ? le->list == list : 0;
}


static inline int list_isempty(const struct list *list)
{
	return list ? list->head == NULL : 1;
}
